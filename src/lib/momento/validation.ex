defmodule Momento.Validation do
  ...
  @spec validate_positive_number(num :: number(), name :: String.t()) ::
          :ok | {:error, Momento.Error.t()}
  defp validate_positive_number(num, name) do
    case validate_number(num, name) do
      :ok ->
        if num > 0.0 do
          :ok
        else
  ...
  defp validate_number(num, _) when is_float(num), do: :ok

  defp validate_number(num, _) when is_integer(num), do: :ok
