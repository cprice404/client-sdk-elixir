defmodule GRPC.Stub do
  ...
  def call(_service_mod, rpc, %{channel: channel} = stream, request, opts) do
    ...
    Logger.info("BEFORE_TELEMETRY! #{:os.system_time(:milli_seconds) - start_time}")

    y = GRPC.Telemetry.client_span(stream, fn ->
      Logger.info("IN_TELEMETRY! #{:os.system_time(:milli_seconds) - start_time}")
      x = do_call(req_stream, stream, request, opts)
      Logger.info("AFTER_CALL! #{:os.system_time(:milli_seconds) - start_time}")
      x
    end)
    Logger.info("AFTER_TELEMETRY! #{:os.system_time(:milli_seconds) - start_time}")
    y
  end
  ...
  defp do_call(
    ...
    Logger.info("DO CALL 1")
    start_time = :os.system_time(:milli_seconds)
    last = fn %{codec: codec, compressor: compressor} = s, _ ->
      Logger.info("DO_CALL: BEFORE ENCODE #{:os.system_time(:milli_seconds) - start_time}")
      message = codec.encode(request)
      Logger.info("DO_CALL: AFTER ENCODE #{:os.system_time(:milli_seconds) - start_time}")
      opts = Keyword.put(opts, :compressor, compressor)

#      x = s
#      |> channel.adapter.send_request(message, opts)
#      |> recv(opts)
      x = channel.adapter.send_request(s, message, opts)
      Logger.info("DO_CALL: AFTER SEND REQUEST #{:os.system_time(:milli_seconds) - start_time}")
      x = recv(x, opts)
      Logger.info("DO_CALL: AFTER RECEIVE REQUEST #{:os.system_time(:milli_seconds) - start_time}")
      x
    end
    ...
    next =
      Enum.reduce(channel.interceptors, last, fn {interceptor, opts}, acc ->
        fn s, r ->
          Logger.info("DO_CALL: CALLING INTERCEPTOR #{:os.system_time(:milli_seconds) - start_time}")
          x = interceptor.call(s, r, acc, opts)
          Logger.info("DO_CALL: AFTER INTERCEPTOR #{:os.system_time(:milli_seconds) - start_time}")
          x
        end
      end)
  ...
  * If the reply is streaming, a enumerable `Stream` will be returned.
  ...
  def recv(%{__interface__: interface} = stream, opts) do
    Logger.info("RECV! INTERFACE: #{inspect(interface)}")
    opts =
      if is_list(opts) do
