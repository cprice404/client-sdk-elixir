...
defmodule GRPC.Client.Adapters.Gun do
  ...
  defp open(host, port, open_opts) do
    ...
    Logger.info("GUN OPEN: #{:os.system_time(:milli_seconds) - start_time}")
    x = :gun.open(String.to_charlist(host), port, open_opts)
    Logger.info("GUN OPEN: BACK FROM ERLANG #{:os.system_time(:milli_seconds) - start_time}")
    x
  end
  ...
  def send_request(stream, message, opts) do
    start_time = :os.system_time(:milli_seconds)
    Logger.info("GUN SEND_REQUEST: BEFORE DO_SEND_REQUEST #{:os.system_time(:milli_seconds) - start_time}")
  ...
  defp do_send_request(
    ...
    start_time = :os.system_time(:milli_seconds)
    Logger.info("GUN DO_SEND_REQUEST: #{:os.system_time(:milli_seconds) - start_time}")
    headers = GRPC.Transport.HTTP2.client_headers_without_reserved(stream, opts)
    {:ok, data, _} = GRPC.Message.to_data(message, opts)
    Logger.info("GUN DO_SEND_REQUEST: BEFORE GUN POST: #{:os.system_time(:milli_seconds) - start_time}")
    x = :gun.post(conn_pid, path, headers, data)
    Logger.info("GUN DO_SEND_REQUEST: AFTER GUN POST: #{:os.system_time(:milli_seconds) - start_time}")
    x
  end
  ...
  def receive_data(
    ...
    Logger.info("GUN RECEIVE DATA 1")
  ...
  def receive_data(stream, opts) do
    start_time = :os.system_time(:milli_seconds)
    Logger.info("GUN RECEIVE_DATA: #{:os.system_time(:milli_seconds) - start_time}")
    %{payload: payload, channel: %{adapter_payload: adapter_payload}} = stream

    Logger.info("GUN RECEIVE_DATA BEFORE HEADERS: #{:os.system_time(:milli_seconds) - start_time}")
    {:ok, headers, _is_fin} = recv_headers(adapter_payload, payload, opts)
    Logger.info("GUN RECEIVE_DATA BEFORE BODY: #{:os.system_time(:milli_seconds) - start_time}")
    {:ok, body, trailers} = recv_body(adapter_payload, payload, opts)
    Logger.info("GUN RECEIVE_DATA BEFORE PARSE: #{:os.system_time(:milli_seconds) - start_time}")
    {:ok, response} = parse_response(stream, headers, body, trailers)
    Logger.info("GUN RECEIVE_DATA AFTER PARSE: #{:os.system_time(:milli_seconds) - start_time}")
    if(opts[:return_headers]) do
...
#    end
  ...
  defp recv_headers(%{conn_pid: conn_pid}, %{stream_ref: stream_ref}, opts) do
    start_time = :os.system_time(:milli_seconds)
    Logger.info("GUN RECEIVE_HEADERS: #{:os.system_time(:milli_seconds) - start_time}")
    x = await(conn_pid, stream_ref, opts[:timeout])
    Logger.info("GUN RECEIVE_HEADERS: AFTER AWAIT: #{:os.system_time(:milli_seconds) - start_time}")
    case x do
  ...
  defp await(conn_pid, stream_ref, timeout) do
    start_time = :os.system_time(:milli_seconds)
    Logger.info("GUN AWAIT: #{:os.system_time(:milli_seconds) - start_time}")
    ...
    Logger.info("GUN AWAIT: BEFORE CALLING ERLANG GUN AWAIT (pid #{inspect(conn_pid)}): #{:os.system_time(:milli_seconds) - start_time}")
    x = :gun.await(conn_pid, stream_ref, timeout)
    Logger.info("GUN AWAIT: AFTER CALLING ERLANG GUN AWAIT: #{:os.system_time(:milli_seconds) - start_time}")
    case x do
